...
const LoadingOverlay: React.FC<LoadingOverlayProps> = ({
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      console.log('🎵 LoadingOverlay completado - Reproduciendo sonido de finalización');
  ...
  useEffect(() => {
    if (progress === 100 && isVisible && !completionTriggeredRef.current) {
      ...
      console.log('🎵 Progreso 100% alcanzado - Reproduciendo sonido de finalización');
  ...
  const getAgentIcon = (agent: string) => {
    switch (agent?.toLowerCase()) {
      case 'planner':
      case 'agente planificador':
        return '🧠';
      case 'codegenerator':
      case 'generador de código':
        return '⚡';
      case 'designarchitect':
      case 'agente de diseño':
      case 'arquitecto de diseño':
        return '🎨';
      case 'codemodifier':
      case 'modificador de código':
        return '🔧';
      case 'fileobserver':
      case 'observador de archivos':
        return '👁️';
      case 'agente corrector':
      case 'codecorrector':
        return '🔍';
      default:
        return '🌩️';
  ...
  const getAgentName = (agent: string) => {
    switch (agent?.toLowerCase()) {
      ...
      case 'agente corrector':
      case 'codecorrector':
        return 'Agente Corrector';
      default:
        return agent || 'Sistema de IA';
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center animate-overlay-appear">
